...
Preprocessing & Normalization / Scaling & Float32 KODAMA-compatible normalization and variable scaling, optional train-derived test transforms, coefficients/statistics, backend identity, and runtime on CPU, CUDA, or Metal. \\
...
\paragraph{Float32 normalization and scaling evidence.}
The typed preprocessing API implements PQN, sum, median, Euclidean-norm, and identity normalization together with centering, autoscaling, range, Pareto, and identity scaling. Independent CPU rows or columns are parallelized; CUDA and Metal compute statistics, deterministic PQN medians, and transforms on device without fallback. Tests cover all ten methods, one/four-thread identity, reconstruction, historical train/test conventions, CUDA agreement, and wrapper calls; the Metal suite contains the corresponding hardware parity checks. The implementation adds no epsilon, imputation, or backend-specific mathematical branch.
