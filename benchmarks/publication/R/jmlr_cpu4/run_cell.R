...
out <- file.path(args$out_root,cell$dataset,cell$representation,cell$classifier,cell$experiment,cell$setting,paste0("seed_",cell$seed))
if(file.exists(file.path(out,"exit_status.txt"))&&identical(readLines(file.path(out,"exit_status.txt")),"0")) quit(save="no")
dir.create(out,recursive=TRUE,showWarnings=FALSE); status <- 1L; started <- Sys.time(); warnings <- character()
withCallingHandlers(tryCatch({
  if(cell$experiment=="classic") {
    ...
    t0 <- proc.time()[["elapsed"]]; fit <- matrix_call(d$x,prepared$graph,cell$classifier,M,Tcycle,5L,nc,100000L,if(nrow(d$x)<40000L)100L else 300L,k,cell$seed,policy,progress=FALSE); wall <- proc.time()[["elapsed"]]-t0
    u0<-proc.time()[["elapsed"]]; umap<-extract_layout(kodamaR::KODAMA.visualization(fit,method="UMAP",k=30L,graph.mode="fuzzy",backend="cpu",n.cores=4L,seed=cell$seed)); usec<-proc.time()[["elapsed"]]-u0
    t0<-proc.time()[["elapsed"]]; opentsne<-extract_layout(kodamaR::KODAMA.visualization(fit,method="opentsne",perplexity=30,backend="cpu",n.cores=4L,seed=cell$seed)); tsec<-proc.time()[["elapsed"]]-t0
    layouts <- list(umap=umap,opentsne=opentsne); visual_seconds<-c(umap=usec,opentsne=tsec)
    layout <- layouts$umap
    fit_stats <- fit_summary(fit); atomic_write_csv(fit_stats,file.path(out,"run_metrics.csv")); atomic_write_csv(cycle_deciles(fit),file.path(out,"cycle_deciles.csv"))
...
},error=function(e) atomic_write_csv(data.frame(cell,status="failed",phase,error=conditionMessage(e),started=as.character(started),ended=as.character(Sys.time())),file.path(out,"metrics.csv"))),warning=function(w){warnings<<-c(warnings,conditionMessage(w));invokeRestart("muffleWarning")})
