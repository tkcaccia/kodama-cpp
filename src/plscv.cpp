...
PLSFit fit_pls_components_cuda_colmajor_y(
  ...
  int max_components,
  int gpu_device,
  bool compute_train_scores = true
) {
...
void candidate_insert_top_float(
...
DenseF cumulative_norm2_float(const DenseF& x) {
  DenseF out(x.rows, x.cols);
  for (int i = 0; i < x.rows; ++i) {
    float acc = 0.0f;
    for (int j = 0; j < x.cols; ++j) {
      acc += x(i, j) * x(i, j);
...
float prefix_row_norm_float(const DenseF& norm2, int row, int ncomp) {
  if (row < 0 || row >= norm2.rows || ncomp < 1 || ncomp > norm2.cols) return 0.0f;
  const float val = norm2(row, ncomp - 1);
  return std::isfinite(val) && val > 0.0f ? std::sqrt(val) : 0.0f;
}
...
DenseF candidate_prefix_centroids_float(
  const DenseF& t_train,
  const DenseF& train_norm2,
  const std::vector<int>& y_codes,
  int n_classes,
  int ncomp
) {
  DenseF cent(n_classes, ncomp);
  std::vector<float> counts(static_cast<std::size_t>(n_classes), 0.0f);
  for (int i = 0; i < t_train.rows; ++i) {
    const int cls = y_codes[static_cast<std::size_t>(i)];
    if (cls < 1 || cls > n_classes) continue;
    const float nrm = prefix_row_norm_float(train_norm2, i, ncomp);
    if (nrm <= 0.0f) continue;
    const int c = cls - 1;
    counts[static_cast<std::size_t>(c)] += 1.0f;
    for (int d = 0; d < ncomp; ++d) cent(c, d) += t_train(i, d) / nrm;
  }
  for (int c = 0; c < n_classes; ++c) {
    if (counts[static_cast<std::size_t>(c)] > 0.0f) {
      const float inv = 1.0f / counts[static_cast<std::size_t>(c)];
      for (int d = 0; d < ncomp; ++d) cent(c, d) *= inv;
    }
    float n2 = 0.0f;
    for (int d = 0; d < ncomp; ++d) n2 += cent(c, d) * cent(c, d);
    const float nrm = std::sqrt(n2);
    if (std::isfinite(nrm) && nrm > 0.0f) {
      for (int d = 0; d < ncomp; ++d) cent(c, d) /= nrm;
    }
  }
  return cent;
...
std::vector<std::vector<int>> predict_pls_cknn_prefix_float(
  const DenseF& t_train,
  const std::vector<int>& y_train_labels,
  const DenseF& t_val,
  const std::vector<int>& classes,
  ...
  if (t_train.cols != t_val.cols) throw std::invalid_argument("float32 PLS-cKNN train/test score dimensions do not match.");
  if (t_train.rows != static_cast<int>(y_train_labels.size())) throw std::invalid_argument("float32 PLS-cKNN label length mismatch.");
  if (components.empty()) throw std::invalid_argument("float32 PLS-cKNN requires at least one component.");
  const int ncomp_total = *std::max_element(components.begin(), components.end());
  if (ncomp_total < 1 || ncomp_total > t_train.cols || ncomp_total > t_val.cols) {
    throw std::invalid_argument("float32 PLS-cKNN component count is out of range.");
  }
  const int n_classes = static_cast<int>(classes.size());
  const int top_m = std::max(1, std::min(top_m_in, n_classes));
  const int knn_k = std::max(1, knn_k_in);
  const float tau = static_cast<float>((std::isfinite(tau_in) && tau_in > 0.0) ? tau_in : 0.2);
  const float alpha = static_cast<float>(std::isfinite(alpha_in) ? alpha_in : 0.5);
  const std::vector<int> y_codes = labels_to_codes(y_train_labels, classes);
  ...
  const DenseF train_norm2 = cumulative_norm2_float(t_train);
  const DenseF test_norm2 = cumulative_norm2_float(t_val);
  std::vector<char> should_score(static_cast<std::size_t>(ncomp_total + 1), 0);
  std::vector<DenseF> centroids_by_comp(static_cast<std::size_t>(ncomp_total + 1));
  for (int a : components) {
    should_score[static_cast<std::size_t>(a)] = 1;
    centroids_by_comp[static_cast<std::size_t>(a)] =
      candidate_prefix_centroids_float(t_train, train_norm2, y_codes, n_classes, a);
  }
  ...
  const int workers = std::max(1, std::min(n_threads, t_val.rows));
  auto worker = [&](int begin, int end) {
    std::vector<float> row_scores(static_cast<std::size_t>(top_m), -std::numeric_limits<float>::infinity());
    std::vector<int> row_index(static_cast<std::size_t>(top_m), -1);
    std::vector<float> rerank_scores(static_cast<std::size_t>(top_m), -std::numeric_limits<float>::infinity());
    std::vector<int> rerank_index(static_cast<std::size_t>(top_m), -1);
    std::vector<float> train_dot(static_cast<std::size_t>(t_train.rows), 0.0f);

    for (int i = begin; i < end; ++i) {
      std::fill(train_dot.begin(), train_dot.end(), 0.0f);
      for (int a = 1; a <= ncomp_total; ++a) {
        const int d = a - 1;
        const float test_val = t_val(i, d);
        for (int r = 0; r < t_train.rows; ++r) train_dot[static_cast<std::size_t>(r)] += t_train(r, d) * test_val;
        if (!should_score[static_cast<std::size_t>(a)]) continue;

        const DenseF& cent = centroids_by_comp[static_cast<std::size_t>(a)];
        std::fill(row_scores.begin(), row_scores.end(), -std::numeric_limits<float>::infinity());
        std::fill(row_index.begin(), row_index.end(), -1);
        const float test_norm = prefix_row_norm_float(test_norm2, i, a);
        for (int cls = 0; cls < n_classes; ++cls) {
          float base = 0.0f;
          if (test_norm > 0.0f) {
            for (int j = 0; j < a; ++j) base += t_val(i, j) * cent(cls, j);
          ...
          candidate_insert_top_float(row_scores, row_index, base, cls + 1);
        }

        std::fill(rerank_scores.begin(), rerank_scores.end(), -std::numeric_limits<float>::infinity());
        std::fill(rerank_index.begin(), rerank_index.end(), -1);
        for (int slot = 0; slot < top_m; ++slot) {
          const int cls = row_index[static_cast<std::size_t>(slot)];
          if (cls < 1 || cls > n_classes) continue;
          ...
          if (!rows.empty()) {
            const int use_k = std::max(1, std::min(knn_k, static_cast<int>(rows.size())));
            std::vector<float> top_vals(static_cast<std::size_t>(use_k), -std::numeric_limits<float>::infinity());
            std::vector<int> top_dummy(static_cast<std::size_t>(use_k), -1);
            for (int train_row : rows) {
              const float train_norm = prefix_row_norm_float(train_norm2, train_row, a);
              const float sim = (test_norm > 0.0f && train_norm > 0.0f) ?
                train_dot[static_cast<std::size_t>(train_row)] / (test_norm * train_norm) :
                0.0f;
              candidate_insert_top_float(top_vals, top_dummy, sim, 0);
            }
            if (!std::isfinite(tau) || tau <= 0.0f) {
              local = 0.0f;
              for (int h = 0; h < use_k; ++h) local += top_vals[static_cast<std::size_t>(h)];
              local /= static_cast<float>(use_k);
            } else {
              const float mx = top_vals[0];
              if (std::isfinite(mx)) {
                float acc = 0.0f;
                for (int h = 0; h < use_k; ++h) acc += std::exp((top_vals[static_cast<std::size_t>(h)] - mx) / tau);
                local = mx + tau * std::log(acc / static_cast<float>(use_k));
              }
            }
          }
          const float score = local + alpha * row_scores[static_cast<std::size_t>(slot)];
          candidate_insert_top_float(rerank_scores, rerank_index, score, cls);
        }
        const int best_code = rerank_index[0] > 0 ? rerank_index[0] : row_index[0];
        const int best_pos = std::max(1, best_code) - 1;
        pred_by_comp[static_cast<std::size_t>(d)][static_cast<std::size_t>(i)] =
...
PLSCVResult run_plscv_cpu(
  ...
  const bool use_float_pls_cpu = true;
  const PLSFoldXCache* fold_cache = (use_fold_cache && !use_float_pls_cpu) ?
    &get_pls_fold_x_cache(x, labels, constrain, options) : nullptr;
  const PLSFoldXCacheF* fold_cache_float = (use_fold_cache && use_float_pls_cpu) ?
  ...
  result.fold_assignments = fold_cache != nullptr ?
    ...
    (fold_cache_float != nullptr ? fold_cache_float->fold_assignments : detail::make_folds(labels, constrain, options.cv));
  ...
  const std::vector<int> fold_ids = fold_cache != nullptr ?
    fold_cache->fold_ids :
    (fold_cache_float != nullptr ? fold_cache_float->fold_ids : detail::sorted_unique_folds(result.fold_assignments));
  ...
  auto process_fold = [&](std::size_t fold_pos) {
    ...
    Dense x_val_storage;
    DenseF x_train_float_storage;
    DenseF x_val_float_storage;
    ...
    const DenseF* x_train_float = nullptr;
    const DenseF* x_val_float = nullptr;
    if (fold_cache != nullptr) {
    ...
    } else if (fold_cache_float != nullptr) {
      const PLSFoldDataF& fold_data = fold_cache_float->folds_data[fold_pos];
      validation = &fold_data.validation;
      train = &fold_data.train;
      x_train_float = &fold_data.x_train;
      x_val_float = &fold_data.x_val;
    } else {
      ...
      if (use_float_pls_cpu) {
        ...
        x_train_float_storage = subset_scale_float(x, train_storage, x_mean, x_scale);
        x_val_float_storage = subset_scale_float(x, validation_storage, x_mean, x_scale);
        x_train_float = &x_train_float_storage;
        x_val_float = &x_val_float_storage;
      } else {
    ...
    if (use_float_pls_cpu) {
      PLSFitF fit = fit_pls_components_labels_float(*x_train_float, y_train_labels, fold_classes, options.max_components);
      ...
      DenseF t_train_full = transform_pls_scores_float(*x_train_float, fit, fit.weights.cols);
      DenseF t_val_full = transform_pls_scores_float(*x_val_float, fit, fit.weights.cols);
      if (mode == PLSMode::PLS_CKNN) {
        std::vector<std::vector<int>> fold_pred_by_comp = predict_pls_cknn_prefix_float(
          t_train_full,
          y_train_labels,
          t_val_full,
          fold_classes,
        ...
        const std::vector<float> y_mean_float = label_means_float(y_train_labels, fold_classes);
        for (int a : eval_components) {
          std::vector<int> fold_pred = mode == PLSMode::PLS_LDA ?
            predict_pls_lda_float(t_train_full, y_train_labels, t_val_full, fold_classes, a) :
            predict_pls_da_labels_float(t_train_full, y_train_labels, t_val_full, fold_classes, y_mean_float, a);
          for (std::size_t i = 0; i < validation->size(); ++i) {
            selected_pred[static_cast<std::size_t>((*validation)[i])] = fold_pred[i];
          }
        }
    ...
    std::vector<double> y_mean;
    Dense y_train = one_hot_centered(labels, *train, fold_classes, y_mean);
    ...
    if (mode == PLSMode::PLS_CKNN) {
      ...
      for (int a : eval_components) {
        std::vector<int> fold_pred = mode == PLSMode::PLS_LDA ?
          predict_pls_lda(fit.train_scores, y_train_labels, t_val_full, fold_classes, a) :
          predict_pls_da_labels(fit.train_scores, y_train_labels, t_val_full, fold_classes, y_mean, a);
        for (std::size_t i = 0; i < validation->size(); ++i) {
          selected_pred[static_cast<std::size_t>((*validation)[i])] = fold_pred[i];
        }
      }
...
PLSCVResult run_plscv_cuda(
  ...
  result.true_labels = labels;
  const bool use_fold_cache = !options.cv.stratified;
  const bool use_float_pls_cuda =
    (mode == PLSMode::PLS_DA || mode == PLSMode::PLS_LDA) &&
    !env_flag_enabled("KODAMA_DISABLE_FLOAT32_PLSLDA");
  const PLSFoldXCache* fold_cache = (use_fold_cache && !use_float_pls_cuda) ?
    &get_pls_fold_x_cache(x, labels, constrain, options) : nullptr;
  const PLSFoldXCacheF* fold_cache_float = (use_fold_cache && use_float_pls_cuda) ?
  ...
  for (std::size_t fold_pos = 0; fold_pos < fold_ids.size(); ++fold_pos) {
    ...
    if (use_float_pls_cuda) {
      ...
      if (mode == PLSMode::PLS_LDA) {
        ...
        const std::vector<float> y_mean_float = label_means_float(y_train_labels, fold_classes);
        for (int a : eval_components) {
          DenseF t_train_prefix = transform_pls_scores_cuda_float(*x_train_float, fit, a, options.gpu_device);
          DenseF t_val_prefix = transform_pls_scores_cuda_float(*x_val_float, fit, a, options.gpu_device);
    ...
    std::vector<double> y_mean;
    const std::vector<double> y_train_colmajor = one_hot_centered_colmajor(labels, *train, fold_classes, y_mean);
    PLSFit fit = fit_pls_components_cuda_colmajor_y(
      *x_train,
      y_train_colmajor.data(),
      static_cast<int>(fold_classes.size()),
      options.max_components,
      options.gpu_device,
    ...
    if (mode == PLSMode::PLS_CKNN) {
    ...
    } else if (mode == PLSMode::PLS_LDA) {
      for (int a : eval_components) {
        const std::vector<int> fold_pred = train_predict_pls_lda_projected_cuda(
          *x_train,
          y_train_labels,
          *x_val,
          fit,
      ...
      Dense t_val_full = transform_pls_scores_cuda(*x_val, fit, fit.weights.cols, options.gpu_device);
      for (int a : eval_components) {
        std::vector<int> fold_pred = predict_pls_da_cuda(fit.train_scores, y_train_labels, t_val_full, fold_classes, y_mean, options.gpu_device);
        for (std::size_t i = 0; i < validation->size(); ++i) {
          selected_pred[static_cast<std::size_t>((*validation)[i])] = fold_pred[i];
        }
      }
