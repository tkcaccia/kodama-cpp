  ...
  while (communities > target) {
    ...
    if (merge_a < 0) {
      ...
      for (int c = 0; c < communities; ++c) {
        if (merge_a < 0 || size[static_cast<std::size_t>(c)] <
            size[static_cast<std::size_t>(merge_a)]) {
          merge_b = merge_a;
          merge_a = c;
        } else if (merge_b < 0 || size[static_cast<std::size_t>(c)] <
                   size[static_cast<std::size_t>(merge_b)]) {
          merge_b = c;
        }
      }
    }
